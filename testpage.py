...
class OperationsHelper(BasePage):
    ...
    def get_about_page_font(self):
        about_page_field = self.find_element(TestSearchLocators.ids['LOCATOR_ABOUT_HEADER'], time=3)
        if about_page_field:
            font = about_page_field.value_of_css_property('font-size')
            logging.debug(f"We find '{font}' in error field {TestSearchLocators.ids['LOCATOR_ABOUT_HEADER']}")
            return font
        else:
    ...
    def get_user_profile(self, token):
        response = requests.get(f"{testdata['profile_user']}/{testdata['id']}", headers={'X-Auth-Token': token})
